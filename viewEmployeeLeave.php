            ...
            <div class="buttons" style="height:20px;">
                ...
                <form id="payForm" >
                    <div style="display:flex; float:right; margin-top:-10px;">
                        ...
                        <input type="hidden" name="leaveID" id="leaveID" value="<?php echo $id; ?>">
                        <input type="hidden" name="payVal" id="payVal" value="">
                        <button type="button" data-val="With Pay" class="btn btn-info btnPay">With Pay</button>
                        <button type="button" data-val="Without Pay" class="btn btn-warning btnPay">Without Pay</button>
                        <button type="button" data-val="Disapprove" class="btn btn-danger adminDis">Disapprove</button>
                    </div>
                </form>
                <?php }} ?>
            </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $("#payForm").on("submit",function(e){
                ...
                let payVal = $("#payVal").val();
                let frmData = new FormData($(this)[0]);

                Swal.fire({
                    title: 'Confirm?',
                    text: 'Set Application For Leave as '+ payVal,
                    type: 'warning',
                    ...
                    if(data.value){
                        Swal.fire({
                            title: '<strong>Additional Notes</strong>',
                            type: 'warning',
                            input:'text',
                            showCloseButton: true,
                            showCancelButton: true,
                            ...
                            if(data.value && data.value != ''){
                                frmData.append("note",data.value);
                                $.ajax({
                                    url:"ajax/savePay.php",
                                    type:"POST",
                                    data: frmData,
            ...
            $(document).on("click",'.adminDis',function(){
                ...
                Swal.fire({
                    ...
                    if(data.value && data.value != ''){
                        frmData.append("leaveID",$("#leaveID").val());
                        frmData.append("remarks",data.value);
                        $.ajax({
                            url:"ajax/adminDisLeave.php",
                            type:"POST",
                            data: frmData,
                            success:function(data){
                                let response = $.parseJSON(data);
